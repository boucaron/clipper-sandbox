...
void ClipperOffset::DoOffset(PathGroup& pathGroup, double delta_)
{
	...
	if (arc_tolerance > 0)
		...
		arc_tol = std::log10(2 + abs_delta) * 0.25; 
		...
		it != pathGroup.paths.data.cend(); ++ it)
	{
		...
		path_in.Trim(min_edge_len);

		size_t pathSize = path_in.size();
		if (pathGroup.end_type == EndType::Polygon && pathSize > 2 &&
			NearEqual(path_in[0], path_in[pathSize - 1], min_edge_len_sqrd))
		{
			path_in.pop_back();
			--pathSize;
		}
...
void ClipperOffset::OffsetOpenPath(EndType et) {
	...
	k = pathSize - 1;
	for (size_t j = k - 1; j > 0; --j) OffsetPoint(j, k);
...
void ClipperOffset::OffsetPoint(size_t j, size_t &k)
{
	...
	if (sin_val < 0.005 && sin_val > -0.005) { k = j; return; } //ie very near colinear
	...
	if(sin_val * delta < 0.0) //ie a concave offset
	 {
		...
		AddPoint(p1);
		if (!NearEqual(p1, p2, 0.5)) { //??make this a member value??
